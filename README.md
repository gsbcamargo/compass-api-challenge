...
Os testes de integração usam Testcontainers, portanto o Docker precisa estar em execução. Se a suíte de testes falhar com `Could not find a valid Docker environment` mesmo com o Docker funcionando normalmente via linha de comando, veja a nota específica sobre isso na seção de Testes, abaixo (é um problema conhecido do Testcontainers com versões recentes do Docker Engine, não um problema deste projeto).
...
- **Testes de integração** (`integration`), usando Testcontainers para subir um PostgreSQL real por teste:
    - `ConcurrentTransferIntegrationTest`: dispara 50 transferências concorrentes entre duas contas e verifica que o saldo total do sistema permanece exatamente o mesmo, e que nenhuma conta jamais fica negativa. É o teste que efetivamente comprova a estratégia de bloqueio descrita acima, e não apenas a lógica de negócio isolada.
...
**Nota sobre um problema conhecido do Testcontainers:** versões recentes do Docker Engine (29 ou superior) exigem API mínima 1.44, mas o cliente `docker-java` embutido no Testcontainers 1.20.x e 1.21.x ainda tenta negociar a partir de uma versão antiga (1.32) e falha antes de sequer tentar a versão correta. É um bug documentado e ainda aberto em vários issues do próprio projeto Testcontainers, não algo específico a este repositório. Foi contornado com um arquivo `src/test/resources/docker-java.properties` contendo `api.version=1.44`, que força o cliente a usar uma versão compatível diretamente. Se os testes de integração falharem com `Could not find a valid Docker environment` mesmo com `docker ps` ou `docker run hello-world` funcionando normalmente, confirme que esse arquivo existe.
